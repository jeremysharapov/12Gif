...
void first_pass() {
  ...
  if (vary && !frame){
    printf("Error: Need to set number of frames to vary\n");
    exit(1);
  }
  else if (frame && !basename){
    strcpy(name, "frame");
    printf("Warning: Basename not set, frame will be used as default\n");
  }
}
...
struct vary_node ** second_pass() {
  ...
  for (i = 0; i < lastop; i++){
	...
	strcpy(new_knob -> name, op[i].op.vary.p -> name);
